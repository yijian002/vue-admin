        	...
        	<el-card class="box-card">
			  ...
			  <div>
			  	<el-form size="mini" v-loading="egsFormLoading" :inline="false" label-width="140px">
			        <el-form-item label="版本判断" required>
			        ...
			        <el-form-item label="低版本生效机器尾号">
			        	...
			        	<el-checkbox v-for="item in checkboxOpts" :label="item" :key="item" :disabled="!egsForm.low_auto_mode" @change="egsForm.egsFormLowNum=handleSetCheckboxArr(item)">{{item}}</el-checkbox>
					    </el-checkbox-group>
			    	</el-form-item>
			        ...
			        <el-form-item label="高版本生效机器尾号">
			        	<el-checkbox-group v-model="egsForm.egsFormHeightNum">
					      <el-checkbox v-for="item in checkboxOpts" :label="item" :key="item" :disabled="!egsForm.high_auto_mode" @change="egsForm.egsFormHeightNum=handleSetCheckboxArr(item)">{{item}}</el-checkbox>
					    </el-checkbox-group>
			    	</el-form-item>
			  ...
			  <div>
			  	<el-form size="mini" v-loading="gamezqFormLoading" :inline="false" label-width="140px">
			        ...
			        <el-form-item label="目录深度">
			        	<el-input-number v-model="gamezqForm.dir_depth" :min="0" :disabled="!gamezqForm.start_disk_scan"></el-input-number>
			    	</el-form-item>

			    	<el-form-item label="版本判断" required>
				        <el-input v-model="gamezqForm.low_version" :disabled="!gamezqForm.start_disk_scan" style="width: 180px;"></el-input>
				        <span class="red f12 lh30">需填写文件版本号，小于等于低版本的受低版本开关控制，大于的受高版本控制</span>
				    </el-form-item>

				    <el-form-item label="低版本开关">
				    	<el-switch v-model="gamezqForm.low_start_process_chain" :disabled="!gamezqForm.start_disk_scan"></el-switch>
				    </el-form-item>
			        <el-form-item label="低版本生效机器尾号">
			        	<el-checkbox-group v-model="gamezqForm.egsFormLowNum">
					      <el-checkbox v-for="item in checkboxOpts" :label="item" :key="item" :disabled="!gamezqForm.start_disk_scan || !gamezqForm.low_start_process_chain" @change="gamezqForm.egsFormLowNum=handleSetCheckboxArr(item)">{{item}}</el-checkbox>
					    </el-checkbox-group>
			    	</el-form-item>

			        <el-form-item label="高版本开关">
			        	<el-switch v-model="gamezqForm.high_start_process_chain" :disabled="!gamezqForm.start_disk_scan"></el-switch>
			        </el-form-item>
			        <el-form-item label="高版本生效机器尾号">
				        <el-checkbox-group v-model="gamezqForm.egsFormHeightNum">
						    <el-checkbox v-for="item in checkboxOpts" :label="item" :key="item" :disabled="!gamezqForm.start_disk_scan || !gamezqForm.high_start_process_chain" @change="gamezqForm.egsFormHeightNum=handleSetCheckboxArr(item)">{{item}}</el-checkbox>
						</el-checkbox-group>
			    	</el-form-item>
	...
	export default {
	    data() {
	        return {
	        	...
	        	gamezqForm: {dir_depth: 5, egsFormLowNum: [], egsFormHeightNum: []},
	        	gamezqList: [],
	    ...
	    methods: {
	        ...
	        saveEgs() {
	        	...
	        	params.low_gray_value = params.egsFormLowNum[params.egsFormLowNum.length-1];
