            ...
            <el-tab-pane label="投 放">
            	<el-form size="mini" label-width="150px" style="width: 400px; margin-top: 10px;">
	                <el-form-item label="限制控制台最低版本">
			            <el-input v-model="formLaunch.version" :maxlength="20" placeholder="空版本为不限制"></el-input>
			        </el-form-item>
			        <el-input type="textarea" v-model="formLaunch.snbids" placeholder="输入网吧帐号，逗号隔开。"></el-input>
...
<script>
	...
	export default {
		...
		methods: {
			addLaunch() {
				if(!this.formLaunch.snbids) {
					this.$message({message: '请输入网吧帐号', type: 'warning'});
					return;
				}

				let snbids = this.formLaunch.snbids.split(','),
					params = {version: this.formLaunch.version, snbids: snbids};
